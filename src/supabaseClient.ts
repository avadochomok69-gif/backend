...
if (!SUPABASE_CONFIGURED) {
  console.warn('[Chomok Fashion] Supabase credentials not set: SUPABASE_URL or SUPABASE_ANON_KEY');
} else {
  try {
    supabase = createClient(SUPABASE_URL as string, SUPABASE_ANON_KEY as string);
    console.log('[Chomok Fashion] Supabase client initialized');
  } catch (err) {
    console.error('[Chomok Fashion] Failed to create Supabase client', err);
    supabase = null;
  }
}

// Export as `any` to avoid repetitive null checks in call sites
